...
public class LivingEntityPatch {
    ...
    @Overwrite(method = "hasEffect", desc = "(Lnet/minecraft/core/Holder;)Z")
    public static boolean overwriteHasEffect(LivingEntity entity, Holder<MobEffect> effect) {
    ...
    @WrapInvoke(method = "tickHeadTurn", desc = "(F)V", target = "net/minecraft/world/entity/Entity/getYRot", targetDesc = "()F")
    public static float onTickHeadTurn(LivingEntity entity, float headRotation, Invocation<LivingEntity, Float> original) throws Exception {
    ...
    @Inject(method = "hurtServer", desc = "(Lnet/minecraft/server/level/ServerLevel;Lnet/minecraft/world/damagesource/DamageSource;F)Z", at = @At(At.Type.HEAD))
    public static void onHurt(LivingEntity entity, ServerLevel level, DamageSource source, float amount, CallbackInfo callbackInfo) {
        if (ZenClient.isReady()) {
