...
implements IHudElement {
    ...
    public void renderGui(GuiGraphics guiGraphics, PoseStack poseStack, float x, float y, float width, float height, float alpha) {
        ...
        if (alpha > 0.1f && iconSize - 4.0f > 0.0f) {
            float renderedSize = iconSize - 4.0f;
            Matrix3x2fStack matrices = guiGraphics.pose();
            matrices.pushMatrix();
            matrices.translate(iconX + 2.0f, iconY + 2.0f);
            matrices.scale(renderedSize / 16.0f, renderedSize / 16.0f);
            guiGraphics.renderItem(blockItem, 0, 0);
            matrices.popMatrix();
        }
    }
