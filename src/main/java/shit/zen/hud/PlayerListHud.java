...
extends HudElement {
    ...
    private List<ItemStack> getPlayerItems(Player player) {
        Stream<ItemStack> armor = Stream.of(
                player.getItemBySlot(EquipmentSlot.FEET),
                player.getItemBySlot(EquipmentSlot.LEGS),
                player.getItemBySlot(EquipmentSlot.CHEST),
                player.getItemBySlot(EquipmentSlot.HEAD));
        Stream<ItemStack> stream = player == mc.player
                ? Stream.concat(player.getInventory().getNonEquipmentItems().stream(), Stream.concat(armor, Stream.of(player.getOffhandItem())))
                : Stream.concat(armor, Stream.of(player.getMainHandItem(), player.getOffhandItem()));
    ...
    public void onGlRender(GlRenderEvent glRenderEvent, float x, float y) {
        ...
        GuiGraphics guiGraphics = glRenderEvent.guiGraphics();
        ...
        if (!this.rightAlignAnim.isDone()) {
            ...
            this.renderLegacy(poseStack, finalX, y, panelWidth, this.rightAlign, globalAlpha * prevSideFade, entryHeight, spacing, cornerRadius, blurStrength, padding);
            this.renderLegacy(poseStack, finalX, y, panelWidth, this.wasRightAligned, globalAlpha * leftFade, entryHeight, spacing, cornerRadius, blurStrength, padding);
        } else {
            this.renderLegacy(poseStack, finalX, y, panelWidth, this.wasRightAligned, globalAlpha, entryHeight, spacing, cornerRadius, blurStrength, padding);
        }
        ...
        for (PlayerListHud.PlayerEntry entry : this.playerEntryList) {
            ...
            if (rightAligned) {
                ...
                for (ItemStack itemStack : entry.items) {
                    Matrix3x2fStack poseStack = guiGraphics.pose();
                    poseStack.pushMatrix();
                    poseStack.translate(rightEdge, itemY);
                    guiGraphics.renderItem(itemStack, 0, 0);
                    poseStack.popMatrix();
