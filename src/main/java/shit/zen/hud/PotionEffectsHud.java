...
extends HudElement {
    public static final class EffectEntry {
        ...
        public void refreshDisplayText() {
            this.effectName = this.effectInstance.getEffect().value().getDisplayName().getString();
        ...
        public void tick() {
            ...
            this.widthAnim.tick();
            if (!this.effectInstance.getEffect().value().isInstantenous()) {
                this.refreshDisplayText();
            }
        }
    }
    ...
    public void onTick(TickEvent tickEvent) {
        ...
        this.effectEntryList.stream().filter(e -> collection.stream().noneMatch(eff -> eff.getEffect().equals(e.getEffect()))).forEach(PotionEffectsHud.EffectEntry::startRemove);
        for (MobEffectInstance mobEffectInstance : collection) {
            Optional<PotionEffectsHud.EffectEntry> existing = this.effectEntryList.stream().filter(e -> !e.removing && e.getEffect().equals(mobEffectInstance.getEffect())).findFirst();
            if (existing.isEmpty()) {
    ...
    private int getEffectColor(Holder<MobEffect> mobEffect) {
        int color = mobEffect.value().getColor();
    ...
    String formatDuration(MobEffectInstance mobEffectInstance) {
        ...
        float ticksPerSecond = mc.level != null ? mc.level.tickRateManager().tickrate() : 20.0f;
        return MobEffectUtil.formatDuration(mobEffectInstance, 1.0f, ticksPerSecond).getString();
    }
