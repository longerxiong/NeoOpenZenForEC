...
extends TargetStyle {
    ...
    public void render(Render2DEvent render2DEvent, LivingEntity livingEntity, SmoothAnimationTimer healthAnim, SmoothAnimationTimer healthLagAnim, float healthPct, float x, float y) {
        ...
        PoseStack poseStack = render2DEvent.poseStack();
        poseStack.pushPose();
        ...
        for (ItemStack itemStack : this.equipmentSlots) {
            if (itemStack != null && !itemStack.isEmpty()) {
                Matrix3x2fStack itemPose = render2DEvent.guiGraphics().pose();
                itemPose.pushMatrix();
                itemPose.translate(itemX, belowNameY);
                itemPose.scale(itemScale, itemScale);
