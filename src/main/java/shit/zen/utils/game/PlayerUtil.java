...
extends ClientBase {
    public static void updateWalkAnim() {
        ...
        mc.player.walkAnimation.update(0.0f, speed, 0.4f);
    }
    ...
    public static int getArmorPoints(LivingEntity livingEntity) {
        ...
        for (EquipmentSlot slot : new EquipmentSlot[]{EquipmentSlot.HEAD, EquipmentSlot.CHEST, EquipmentSlot.LEGS, EquipmentSlot.FEET}) {
            ...
            totalDefense += getArmorDefense(itemStack.getItem());
        }
        return totalDefense;
    }

    private static int getArmorDefense(Item item) {
        if (item == Items.LEATHER_HELMET || item == Items.LEATHER_BOOTS) return 1;
        if (item == Items.LEATHER_CHESTPLATE) return 3;
        if (item == Items.LEATHER_LEGGINGS) return 2;
        if (item == Items.CHAINMAIL_HELMET || item == Items.CHAINMAIL_BOOTS || item == Items.GOLDEN_HELMET || item == Items.GOLDEN_BOOTS || item == Items.IRON_HELMET || item == Items.IRON_BOOTS) return 2;
        if (item == Items.CHAINMAIL_CHESTPLATE || item == Items.GOLDEN_CHESTPLATE || item == Items.IRON_CHESTPLATE) return 5;
        if (item == Items.CHAINMAIL_LEGGINGS || item == Items.GOLDEN_LEGGINGS || item == Items.IRON_LEGGINGS) return 4;
        if (item == Items.DIAMOND_HELMET || item == Items.DIAMOND_BOOTS || item == Items.NETHERITE_HELMET || item == Items.NETHERITE_BOOTS) return 3;
        if (item == Items.DIAMOND_LEGGINGS || item == Items.NETHERITE_LEGGINGS) return 6;
