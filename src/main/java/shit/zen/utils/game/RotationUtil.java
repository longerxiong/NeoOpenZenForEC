...
extends ClientBase {
    ...
    public static float[] getBallisticAngles(Vec3 target) {
        ...
        block0: for (float pitch = 90.0f; pitch >= -90.0f; pitch -= 0.5f) {
            ...
            for (int i = 0; i < 300; ++i) {
                ...
                if (projectilePos.y < (double)(mc.level.dimensionType().minY() - 10)) continue block0;
    ...
    public static RotationUtil.BestHitInfo getBestHit(Entity entity) {
        return RotationUtil.getBestHit(entity, 3.0);
    }
    ...
    public static RotationUtil.BestHitInfo getBestHit(Entity entity, double range, boolean ignoreBlocks) {
        ...
        double step = 0.1;
        LinkedHashSet<Vec3> samplePoints = new LinkedHashSet<>();
    ...
    private static boolean isHitValid(Vec3 eyePos, HitResult hitResult, Entity entity, double range) {
        if (hitResult == null) {
            return false;
        }
        if (hitResult.getType() == HitResult.Type.ENTITY && ((EntityHitResult)hitResult).getEntity() == entity) {
            Vec3 hitLocation = hitResult.getLocation();
            return RotationUtil.isInsideAABB(RotationUtil.getEntityBB(entity), eyePos)
                    || hitLocation.distanceTo(eyePos) <= range;
        }
        return false;
    }
    ...
    public static HitResult performRaycast(Rotation rotation, double range, boolean ignoreBlocks) {
        ...
        Vec3 direction = RotationUtil.getDirection(rotation.getYaw(), rotation.getPitch());
        Vec3 endPos = eyePos.add(direction.x * range, direction.y * range, direction.z * range);
        EntityHitResult entityHitResult = ProjectileUtil.getEntityHitResult(mc.player, eyePos, endPos, expandedBB = mc.player.getBoundingBox().expandTowards(direction.scale(range)).inflate(1.0, 1.0, 1.0), entity -> !entity.isSpectator() && entity.isPickable(), maxRangeSqr);
        if (entityHitResult != null) {
            Vec3 hitLocation = entityHitResult.getLocation();
            double hitDistSqr = eyePos.distanceToSqr(hitLocation);
            if (hitDistSqr <= maxRangeSqr || hitResult == null) {
    ...
    public static Direction getNearestDirection(double x, double y, double z) {
        ...
        for (Direction dir : Direction.values()) {
            // 只检查六个主轴方向（UP, DOWN, NORTH, SOUTH, WEST, EAST）
            float dot = (float)(x * dir.getStepX() + y * dir.getStepY() + z * dir.getStepZ());
            if (dot > maxDot) {
                maxDot = dot;
                closest = dir;
            }
