...
public final class TextGlow {
    ...
    public static void drawBackground(DrawContext drawContext, float x, float y, float width, float height, float radius, float alpha) {
        int darkAlpha = TextGlow.clampAlpha(150.0f * alpha);
        int lightAlpha = TextGlow.clampAlpha(35.0f * alpha);
        drawContext.drawRoundedRect(RoundedRectangle.ofXYWHR(x, y, width, height, radius),
                new Paint().setColor(new Color(24, 24, 24, darkAlpha).getRGB()));
        drawContext.drawRoundedRect(RoundedRectangle.ofXYWHR(x, y, width, height, radius),
                new Paint().setColor(new Color(255, 255, 255, lightAlpha).getRGB()));
    }
