...
implements Closeable {
    ...
    public void drawString(DrawContext drawContext, String text, double x, double y, int color) {
        ...
        this.drawStringRGB(drawContext, text, (float)x, (float)y, r, g, b, a);
    }

    public void drawStringShadow(DrawContext drawContext, String text, double x, double y, int color) {
        this.drawString(drawContext, text, x, y, color);
    }

    public void drawStringWithShadow(DrawContext drawContext, String text, double x, double y, int color) {
        this.drawStringColor(drawContext, text, x + 0.5, y + 0.5, SHADOW_COLOR);
        this.drawString(drawContext, text, x, y, color);
    }

    public void drawStringColor(DrawContext drawContext, String text, double x, double y, Color color) {
        this.drawStringRGB(drawContext, text, (float)x, (float)y,
    ...
    public void drawStringRGB(DrawContext drawContext, String text, float x, float y, float r, float g, float b, float a) {
        this.drawStringRGBFull(drawContext, text, x, y, r, g, b, a, false, 0);
    }

    public void drawStringRGBFull(DrawContext drawContext, String text, float x, float y, float baseR, float baseG, float baseB, float alpha, boolean rainbow, int rainbowOffset) {
        if (this.preloadFuture != null && !this.preloadFuture.isDone()) {
            try {
                ...
                throw new IllegalStateException("Failed to preload font glyphs", exception.getCause());
            }
        }
        ...
        float curG = baseG;
        float curB = baseB;
        float normalizedAlpha = alpha > 1.0f ? alpha / 255.0f : alpha;
        drawContext.save();
        drawContext.translate((float)MathUtil.round(x, 1), (float)MathUtil.round(y - 1.0f, 1));
        drawContext.scale(1.0f / this.scale, 1.0f / this.scale);
        try {
            float penX = 0.0f;
            float penY = 0.0f;
            ...
            for (int i = 0; i < chars.length; i++) {
                ...
                if (inFormatting) {
                    ...
                    if (MC_COLOR_CODES.containsKey(upper)) {
                        ...
                        curR = (float)(packed >> 16 & 0xFF) / 255.0f;
                        curG = (float)(packed >> 8 & 0xFF) / 255.0f;
                        curB = (float)(packed & 0xFF) / 255.0f;
                    } else if (upper == 'R') {
                        curR = baseR;
                        curG = baseG;
                        curB = baseB;
                ...
                if (glyph.value() != ' ') {
                    ...
                    int packedColor = Math.round(normalizedAlpha * 255.0f) << 24
                            | Math.round(curR * 255.0f) << 16
                            | Math.round(curG * 255.0f) << 8
                            | Math.round(curB * 255.0f);
                    drawContext.drawTexture(
                            new Texture(page.textureLocation, page.imageWidth, page.imageHeight),
                            Rectangle.ofXYWH(glyph.u(), glyph.v(), glyph.width(), glyph.height()),
                            Rectangle.ofXYWH(penX, penY, glyph.width(), glyph.height()),
                            new Paint().setColor(packedColor));
                }
    ...
    public void drawStringRGBFull(PoseStack poseStack, String text, float x, float y, float baseR, float baseG, float baseB, float alpha, boolean rainbow, int rainbowOffset) {
        DrawContext currentCanvas = Renderer.getCanvas();
        if (currentCanvas != null) {
            int alphaByte = Math.round(Math.max(0.0f, Math.min(1.0f, alpha > 1.0f ? alpha / 255.0f : alpha)) * 255.0f);
            int color = alphaByte << 24
                    | Math.round(Math.max(0.0f, Math.min(1.0f, baseR)) * 255.0f) << 16
                    | Math.round(Math.max(0.0f, Math.min(1.0f, baseG)) * 255.0f) << 8
                    | Math.round(Math.max(0.0f, Math.min(1.0f, baseB)) * 255.0f);
            Matrix4f matrix = poseStack.last().pose();
            currentCanvas.save();
            try {
                currentCanvas.getGuiGraphics().pose().mul(new Matrix3x2f(
                        matrix.m00(), matrix.m01(), matrix.m10(), matrix.m11(), matrix.m30(), matrix.m31()));
                this.drawString(currentCanvas, text, x, y, color);
            } finally {
        ...
        int alphaByte = Math.round(Math.max(0.0f, Math.min(1.0f, alpha > 1.0f ? alpha / 255.0f : alpha)) * 255.0f);
        poseStack.pushPose();
        poseStack.translate(MathUtil.round(x, 1), MathUtil.round(y - 1.0f, 1), 0.0);
        ...
        try {
            ...
            synchronized (this.glyphPageMap) {
                    ...
                    if (inFormatting) {
                        ...
                        if (glyph != null) {
                            if (glyph.value() != ' ') {
                                ...
                                this.glyphPageMap.computeIfAbsent(textureLocation, key -> new ObjectArrayList<>())
                ...
                for (ResourceLocation textureLocation : this.glyphPageMap.keySet()) {
                    ...
                    var bufferBuilder = Tesselator.getInstance().begin(RenderType.celestial(textureLocation).mode(), RenderType.celestial(textureLocation).format());
                    for (GlyphEntry entry : entries) {
                        ...
                        int color = alphaByte << 24
                                | Math.round(entry.r * 255.0f) << 16
                                | Math.round(entry.g * 255.0f) << 8
                                | Math.round(entry.b * 255.0f);
                        bufferBuilder.addVertex(pose, entry.atX, entry.atY + glyphHeight, 0.0F).setUv(u1, v2).setColor(color);
                        bufferBuilder.addVertex(pose, entry.atX + glyphWidth, entry.atY + glyphHeight, 0.0F).setUv(u2, v2).setColor(color);
                        bufferBuilder.addVertex(pose, entry.atX + glyphWidth, entry.atY, 0.0F).setUv(u2, v1).setColor(color);
                        bufferBuilder.addVertex(pose, entry.atX, entry.atY, 0.0F).setUv(u1, v1).setColor(color);
                    }
                    MeshData meshData = bufferBuilder.build();
                    if (meshData != null) {
                        RenderType.celestial(textureLocation).draw(meshData);
                    }
                }
