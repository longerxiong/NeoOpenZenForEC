    ...
    public static void uploadTexture(ResourceLocation resourceLocation, BufferedImage source) {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                int argb = colorModel.getAlpha(pixelData) << 24
                        | colorModel.getRed(pixelData) << 16
                        | colorModel.getGreen(pixelData) << 8
                        | colorModel.getBlue(pixelData);
        ...
        Runnable register = () -> {
            try {
                DynamicTexture dynamicTexture = new DynamicTexture(
                        () -> "OpenZen glyph atlas " + resourceLocation,
                        nativeImage);
                dynamicTexture.setFilter(true, false);
                dynamicTexture.setClamp(true);
                ClientBase.mc.getTextureManager().register(resourceLocation, dynamicTexture);
            } catch (Throwable throwable) {
                nativeImage.close();
                throw new IllegalStateException("Failed to upload glyph atlas " + resourceLocation, throwable);
            }
        };
        if (RenderSystem.isOnRenderThread()) {
            ...
            ClientBase.mc.execute(() -> {
                try {
                    ...
                    upload.completeExceptionally(throwable);
                }
            });
            upload.join();
        }
