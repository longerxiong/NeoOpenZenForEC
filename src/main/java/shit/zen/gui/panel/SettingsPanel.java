...
extends ClientBase {
    ...
    public void render(GuiGraphics guiGraphics, int originX, int originY, int mouseX, int mouseY, Module module, float scale, float alpha) {
        ...
        try {
            ...
            this.drawRoundedRect(guiGraphics, panelX, panelY, panelWidth, panelHeight, 4.0f * scale, this.applyAlpha(PANEL_BG_COLOR, alpha));
    ...
    private void renderScrollbar(GuiGraphics guiGraphics, int panelX, int panelY, int panelHeight, float scale, float alpha) {
        ...
        this.drawRoundedRect(guiGraphics, thumbX, thumbY, thumbWidth, thumbHeight, thumbWidth / 2.0f, thumbColor);
    }
    ...
    private void renderToggleButton(GuiGraphics guiGraphics, int toggleX, int toggleY, float enabledFactor, float hoverFactor, boolean enabled, float scale, float alpha) {
        ...
        if (enabled) {
            ...
            this.drawRoundedRect(guiGraphics, (float)toggleX - scale, (float)toggleY - scale, (float)toggleWidth + 2.0f * scale, (float)toggleHeight + 2.0f * scale, (float)toggleHeight / 2.0f, overlayColor);
        }
        this.drawRoundedRect(guiGraphics, toggleX, toggleY, toggleWidth, toggleHeight, (float)toggleHeight / 2.0f, trackColorAlpha);
        ...
        if (hoverFactor > 0.0f) {
            ...
            this.drawRoundedRect(guiGraphics, knobX - 1, knobY - 1, highlightSize, highlightSize, highlightSize / 2.0f, this.applyAlpha(highlightColor, alpha));
        }
        this.drawRoundedRect(guiGraphics, knobX, knobY, overlayColor, overlayColor, (float)overlayColor / 2.0f, this.applyAlpha(-1, alpha));
    }
