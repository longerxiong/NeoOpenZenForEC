...
implements SettingRenderer {
    ...
    public int render(GuiGraphics guiGraphics, Setting<?> setting, int x, int y, int width, int mouseX, int mouseY, float alpha, float scale) {
        ...
        FontRenderer stateFont = FontPresets.axiformaBold(11.0f * scale);
        String stateText = booleanSetting.getValue() ? "ON" : "OFF";
        int stateColor = booleanSetting.getValue() ? COLOR_ON : COLOR_OFF;
        float stateX = toggleX - GlHelper.getStringWidth(stateText, stateFont) - 5.0f * scale;
        float stateY = (float)y + (float)rowHeight / 2.0f - stateFont.getMetrics().capHeight() / 2.0f;
        GlHelper.drawText(stateText, stateX, stateY, stateFont, this.applyAlpha(stateColor, alpha));
        return rowHeight;
    }
    ...
    private void drawToggle(GuiGraphics guiGraphics, int toggleX, int toggleY, float onFactor, float hoverFactor, float alpha, float scale) {
        ...
        if (onFactor > 0.01f) {
            ...
            this.drawRoundedRect(guiGraphics, (float)toggleX - scale, (float)toggleY - scale, (float)toggleWidth + 2.0f * scale, (float)toggleHeight + 2.0f * scale, (float)toggleHeight / 2.0f, this.applyAlpha(overlayColor, alpha));
        }
        this.drawRoundedRect(guiGraphics, toggleX, toggleY, toggleWidth, toggleHeight, (float)toggleHeight / 2.0f, this.applyAlpha(trackColor, alpha));
        ...
        if (hoverFactor > 0.0f) {
            ...
            this.drawRoundedRect(guiGraphics, knobX - 1, knobY - 1, knobSize + 2, knobSize + 2, (float)(knobSize + 2) / 2.0f, this.applyAlpha(highlightColor, alpha));
        }
        this.drawRoundedRect(guiGraphics, knobX, knobY, knobSize, knobSize, (float)knobSize / 2.0f, this.applyAlpha(knobColor, alpha));
    }

    private void drawRoundedRect(GuiGraphics guiGraphics, float x, float y, float width, float height, float radius, int color) {
        Renderer.render(guiGraphics, drawContext -> drawContext.drawRoundedRect(
                RoundedRectangle.ofXYWHR(x, y, width, height, radius),
