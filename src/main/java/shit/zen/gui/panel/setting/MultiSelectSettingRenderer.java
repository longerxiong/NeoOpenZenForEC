    ...
    public int render(GuiGraphics guiGraphics, Setting<?> setting, int x, int y, int width, int mouseX, int mouseY, float alpha, float scale) {
        ...
        for (String option : multiSelectSetting.getOptions()) {
            ...
            if (selectedAmount > 0.01f) {
                ...
                this.drawRoundedRect(guiGraphics, (float)boxX - scale, (float)boxY - scale, (float)boxSize + 2.0f * scale, (float)boxSize + 2.0f * scale, 2.0f * scale, color);
            }
            this.drawRoundedRect(guiGraphics, boxX, boxY, boxSize, boxSize, 2.0f * scale, this.applyAlpha(-5592406, alpha));
            if ((double)selectedAmount > 0.01) {
                float inset = (1.0f - selectedAmount) * ((float)boxSize / 2.0f);
                this.drawRoundedRect(guiGraphics, (float)boxX + inset, (float)boxY + inset, (float)boxSize - inset * 2.0f, (float)boxSize - inset * 2.0f, scale, this.applyAlpha(COLOR_SELECTED, alpha));
            }
