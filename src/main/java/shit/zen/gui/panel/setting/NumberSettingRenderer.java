...
implements SettingRenderer {
    ...
    private void drawRect(GuiGraphics guiGraphics, float x, float y, float width, float height, int color) {
        Renderer.render(guiGraphics, drawContext -> drawContext.drawRectXYWH(
                x, y, width, height, new Paint().setColor(color)));
    }
    ...
    private void drawNumberWidget(GuiGraphics guiGraphics, NumberSetting numberSetting, int x, int y, int width, int widgetX, int widgetY, int widgetWidth, int widgetHeight, int iconX, int iconY, boolean editing, float alpha, float scale) {
        ...
        int sidePadding = Math.round(12.0f * scale);
        this.drawRoundedRect(guiGraphics, widgetX, widgetY, widgetWidth, widgetHeight, 5.0f * scale, this.applyAlpha(0x50F5F5F5, alpha));
        int middleX = widgetX + sidePadding;
        int middleWidth = widgetWidth - sidePadding * 2;
        this.drawRoundedRect(guiGraphics, middleX, widgetY, middleWidth, widgetHeight, 0.0f, this.applyAlpha(1086900424, alpha));
        ...
        if (editing) {
            ...
            this.drawRect(guiGraphics, caretX, displayY, Math.max(1.0f, Math.round(scale)), caretHeight, caretColor);
        } else {
