...
extends ClientBase {
    ...
    private void drawBackground(GuiGraphics guiGraphics, int screenWidth, int screenHeight) {
        ...
        Renderer.render(guiGraphics, drawContext -> drawContext.drawRoundedRect(
                RoundedRectangle.ofXYWHR(0.0f, 0.0f, screenWidth, screenHeight, 0.0f),
                new Paint().setColor(color.getRGB())));
    }
    ...
    private void drawGlow(GuiGraphics guiGraphics, int boxX, int boxY, float boxWidth, float boxHeight, float scale) {
        Renderer.render(guiGraphics, drawContext -> TextGlow.drawBackground(
                drawContext, boxX, boxY, boxWidth, boxHeight, 12.0f * scale, this.alpha));
    }
