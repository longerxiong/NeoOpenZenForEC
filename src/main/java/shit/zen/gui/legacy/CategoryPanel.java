...
public class CategoryPanel {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        drawContext.drawRectXYWH(this.x, this.y, this.width, this.rowHeight,
                new shit.zen.render.Paint().setColor(new Color(16, 16, 20, 245).getRGB()));
        ...
        FontStore.ICON_30.drawStringWithShadow(drawContext, iconChar, this.x + 4, iconY, -1);
        float labelY = (float)this.y + ((float)this.rowHeight / 2.0f - FontStore.OPENSANS_18.getFontHeight() / 2.0f) - 0.5f;
        FontStore.OPENSANS_18.drawStringWithShadow(drawContext, this.category.displayName, this.x + this.rowHeight + 4, labelY, -1);
        PoseStack legacyPose = new PoseStack();
        for (ModuleButton moduleButton : this.moduleButtons) {
            moduleButton.render(legacyPose, mouseX, mouseY, partialTicks);
        }
    }
