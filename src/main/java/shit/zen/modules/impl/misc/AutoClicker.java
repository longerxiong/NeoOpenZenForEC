    ...
    private void doLeftClick() {
        ...
        switch ((String) this.clickMethod.getValue()) {
            ...
            case "Key":
                ...
                break;
            case "Mouse":
                NeoForge.EVENT_BUS.post(new InputEvent.MouseButton.Pre(mc.options.keyAttack.getKey().getValue(), 1, 0));
                KeyMapping.click(mc.options.keyAttack.getKey());
                NeoForge.EVENT_BUS.post(new InputEvent.MouseButton.Post(mc.options.keyAttack.getKey().getValue(), 1, 0));
                break;
        }
        this.leftClickTimer.reset();
    }
    ...
    private void doRightClick() {
        ...
        switch ((String) this.clickMethod.getValue()) {
            ...
            case "Key":
                ...
                break;
            case "Mouse":
                NeoForge.EVENT_BUS.post(new InputEvent.MouseButton.Pre(mc.options.keyUse.getKey().getValue(), 1, 0));
                KeyMapping.click(mc.options.keyUse.getKey());
                NeoForge.EVENT_BUS.post(new InputEvent.MouseButton.Post(mc.options.keyUse.getKey().getValue(), 1, 0));
                break;
        }
