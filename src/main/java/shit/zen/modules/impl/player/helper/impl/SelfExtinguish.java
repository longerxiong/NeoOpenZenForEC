...
//                    mc.player.getInventory().selected = this.waterBucketSlot;
                    mc.player.getInventory().setSelectedSlot(this.waterBucketSlot);
                    this.isAiming = true;
...
//                mc.player.getInventory().setSelectedSlot(this.savedSlot);
    ...
    public InteractionResult useItem(Player player, Level level, InteractionHand interactionHand) {
        ...
        PacketUtil.sendPredictive(seq -> new ServerboundUseItemPacket(interactionHand, seq, mc.player.getXRot(), mc.player.getYRot()));
        ItemStack heldStack = player.getItemInHand(interactionHand);
        if (player.getCooldowns().isOnCooldown(heldStack.getItem().getDefaultInstance())) {
            return InteractionResult.PASS;
        }
        InteractionResult forgeResult = heldStack.getItem().use(level, player, interactionHand);
        if (forgeResult != null) {
            return forgeResult;
        }
        InteractionResult result = heldStack.use(level, player, interactionHand);
        if (heldStack.isEmpty()) {
            NeoForge.EVENT_BUS.post(new PlayerDestroyItemEvent(player, heldStack, interactionHand));
        }
        return result;
    }
