...
public class InventoryManager extends Module {
    ...
    private final NumberSetting actionDelaySetting = new NumberSetting(
            "Delay", 200, 0, 500, 10, () -> !this.isInstant());
    private final NumberSetting sprintDelayTicksSetting = new NumberSetting(
            "Open Delay", 2, 0, 10, 1, () -> !this.isInstant());
    private final NumberSetting dropDelaySetting = new NumberSetting(
            "Drop Delay", 200, 0, 500, 10, () -> !this.isInstant());
    ...
    private boolean isActionReady() {
        return this.isInstant() || actionTimer.hasPassed(this.actionDelaySetting.getValue().intValue());
    }

    private boolean isDropReady() {
        return this.isInstant()
                || this.fastThrowSetting.getValue()
                || actionTimer.hasPassed(this.dropDelaySetting.getValue().intValue());
    }
    ...
    public void onPacket(PacketEvent event) {
        ...
        if (isPerformingAction && packet instanceof ServerboundPlayerInputPacket input) {
            if (mc.player.isSprinting()) {
                ...
                PacketUtil.sendQueued(new ServerboundPlayerInputPacket(input.input()));
                return;
            }
        }
        ...
        if (!this.isInstant() && !externalContainerOpen
...
//            ChatUtil.print("Releasing Packet: " + packet.getClass().getName());
    ...
    public void onMotionManage(MotionEvent event) {
        ...
        if (externalContainerOpen
                || (!this.isInstant() && ChestStealer.isRateLimited())
        ...
        if (this.inventoryOnlySetting.getValue() && mc.screen instanceof InventoryScreen) {
            ...
            if (!this.isInstant()
                    && this.sprintWaitTicks < this.sprintDelayTicksSetting.getValue().intValue()) {
                return;
            }
        }
        ...
        if (this.isInstant()) {
            while (!this.pendingPackets.isEmpty()) {
                PacketUtil.sendQueued(this.pendingPackets.poll());
            }
            boolean performed = false;
            for (int action = 0; action < MAX_INSTANT_ACTIONS; action++) {
                int previousSequence = this.actionSequence;
                boolean reportedAction = this.performInventoryAction();
                boolean performedAction = this.actionSequence != previousSequence;
                if (!reportedAction && !performedAction) {
                    break;
                }
                performed |= performedAction;
            }
            isPerformingAction = performed;
            this.skipNextTick = !performed;
        } else if (this.performInventoryAction()) {
    ...
    private boolean performInventoryAction() {
        ...
        if (this.autoArmorSetting.getValue()) {
            for (int i = 0; i < 4; i++) {
                ...
                if (!equipped.isEmpty()
                        && (equipped.is(ItemTags.HEAD_ARMOR) || equipped.is(ItemTags.CHEST_ARMOR) || equipped.is(ItemTags.LEG_ARMOR) || equipped.is(ItemTags.FOOT_ARMOR))
                        && this.isActionReady()) {
                    EquipmentSlot slot = switch (36 + i) {
                        case 39 -> EquipmentSlot.HEAD;
                        case 38 -> EquipmentSlot.CHEST;
                        case 37 -> EquipmentSlot.LEGS;
                        default -> EquipmentSlot.FEET;
                    };
                    if (ItemUtil.getBestArmorScore(slot) > ItemUtil.getArmorScore(equipped)) {
                        mc.gameMode.handleInventoryMouseClick(
                                mc.player.inventoryMenu.containerId,
                                4 + (4 - i), 1, ClickType.THROW, mc.player);
                        this.markInventoryAction();
                        return true;
                    }
                }
            }
            ...
            for (int i = 0; i < 36; i++) {
                ...
                if (candidate.isEmpty() || !(candidate.is(ItemTags.HEAD_ARMOR) || candidate.is(ItemTags.CHEST_ARMOR) || candidate.is(ItemTags.LEG_ARMOR) || candidate.is(ItemTags.FOOT_ARMOR))) continue;
                float candidateScore = ItemUtil.getArmorScore(candidate);
                EquipmentSlot equipSlot = candidate.is(ItemTags.HEAD_ARMOR) ? EquipmentSlot.HEAD : candidate.is(ItemTags.CHEST_ARMOR) ? EquipmentSlot.CHEST : candidate.is(ItemTags.LEG_ARMOR) ? EquipmentSlot.LEGS : EquipmentSlot.FEET;
                boolean isBest = ItemUtil.getBestArmorScore(equipSlot) == candidateScore;
                boolean betterThanEquipped = ItemUtil.getEquippedArmorScore(equipSlot) < candidateScore;
        ...
        if ("Golden Apple".equals(offhandPref)) {
            ...
            if (slot != -1 && this.isActionReady()) {
                if (offhand.getItem() != Items.GOLDEN_APPLE) {
                    this.moveToOffhand(slot);
                    return true;
                }
                ItemStack invStack = mc.player.getInventory().getItem(slot);
        ...
        if (this.swordSlotSetting.getValue().intValue() != 0) {
            ...
            if (bestSword != null) {
                float currentDamage = current.is(ItemTags.SWORDS)
                        ? ItemUtil.getSwordDamage(current)
                        : ItemUtil.getAxeDamage(current);
                float candidateDamage = bestSword.is(ItemTags.SWORDS)
        ...
        if (this.pickaxeSlotSetting.getValue().intValue() != 0) {
            ...
            ItemStack current = mc.player.getInventory().getItem(slot);
            if (bestPickaxe != null
                    && bestPickaxe.is(ItemTags.PICKAXES)
        ...
        List<Integer> order = IntStream.range(0, 36)
                .boxed().collect(Collectors.toList());
        Collections.shuffle(order);
        for (Integer idx : order) {
            ItemStack stack = mc.player.getInventory().getItem(idx);
            if (!stack.isEmpty() && !this.isUsefulItem(stack) && this.throwItem(stack)) {
                return true;
            }
        }
        return false;
    ...
    private boolean swapToSlot(int targetSlot, ItemStack stack) {
        if (mc.gameMode == null || mc.player == null) return false;
        ItemStack current = mc.player.getInventory().getItem(targetSlot);
    ...
    private boolean swapItemToSlot(int targetSlot, Item item) {
        ...
        if (source == -1) return false;
        ItemStack sourceStack = mc.player.getInventory().getItem(source);
    ...
    public boolean isUsefulItem(ItemStack stack) {
        ...
        if (stack.is(ItemTags.HEAD_ARMOR) || stack.is(ItemTags.CHEST_ARMOR) || stack.is(ItemTags.LEG_ARMOR) || stack.is(ItemTags.FOOT_ARMOR)) {
            ...
            EquipmentSlot equipSlot = stack.is(ItemTags.HEAD_ARMOR) ? EquipmentSlot.HEAD : stack.is(ItemTags.CHEST_ARMOR) ? EquipmentSlot.CHEST : stack.is(ItemTags.LEG_ARMOR) ? EquipmentSlot.LEGS : EquipmentSlot.FEET;
