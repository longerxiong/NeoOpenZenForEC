...
public class MidPearl extends Module {
    ...
    public void onDisable() {
        if (this.pearlSavedSlot != -1 && mc.player != null) {
            mc.player.getInventory().setSelectedSlot(this.pearlSavedSlot);
        }
        if (this.rodSavedSlot != -1 && mc.player != null) {
            mc.player.getInventory().setSelectedSlot(this.rodSavedSlot);
        }
        this.reset();
    }
    ...
    private void selectPearlSlot(int slot) {
        ...
        this.rodSavedSlot = mc.player.getInventory().getSelectedSlot();
        mc.player.getInventory().setSelectedSlot(slot);
    }

    private void throwPearl() {
        if (mc.player.getMainHandItem().getItem() == Items.ENDER_PEARL) {
            PacketUtil.sendPredictive(n -> new ServerboundUseItemPacket(InteractionHand.MAIN_HAND, n, mc.player.getYRot(), mc.player.getXRot()));
        ...
        if (pressed) {
            if (!this.pearlSlotSelected) {
                ...
                if (slot != -1 && slot < 9) {
                    ...
                    this.pearlSavedSlot = mc.player.getInventory().getSelectedSlot();
