...
//        mc.player.getInventory().selected = this.blockSlot;
        mc.player.getInventory().setSelectedSlot(this.blockSlot);
    ...
    private int findBlockSlot() {
        ...
        for (int slot = 0; slot < 9; ++slot) {
            ...
            if (!(itemStack.getItem() instanceof BlockItem) || !(block = ((BlockItem)itemStack.getItem()).getBlock()).defaultBlockState().isSolid() || block.defaultBlockState().getSoundType().getPlaceSound().equals(SoundEvents.WOOD_PLACE)) continue;
            return slot;
        }
        return -1;
    }
