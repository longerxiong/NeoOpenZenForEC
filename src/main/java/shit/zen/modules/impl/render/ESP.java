...
public class ESP extends Module {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        DrawContext drawContext = new DrawContext(event.guiGraphics());
        try (Paint paint = new Paint()) {
            for (Map.Entry<Entity, Pair<Vector4d, Boolean>> entry : this.entityBoxPositions.entrySet()) {
                ...
                this.drawFilledRect2D(drawContext, paint, x1, y1, x2, y2, color);
                if (entity instanceof LivingEntity le && this.showHealthBarSetting.getValue()) {
                    this.drawHealthBar(drawContext, paint, le, v, color);
                }
            }
        }
    }

    private void drawFilledRect2D(DrawContext context, Paint paint, float x1, float y1, float x2, float y2, Color color) {
        this.drawRect(context, paint, x1 - 1.0f, y1, x1 + 0.5f, y2 + 0.5f, Color.BLACK);
        this.drawRect(context, paint, x1 - 1.0f, y1 - 0.5f, x2 + 0.5f, y1 + 1.0f, Color.BLACK);
        this.drawRect(context, paint, x2 - 0.5f, y1, x2 + 0.5f, y2 + 0.5f, Color.BLACK);
        this.drawRect(context, paint, x1 - 1.0f, y2 - 0.5f, x2 + 0.5f, y2 + 0.5f, Color.BLACK);
        this.drawRect(context, paint, x1 - 0.5f, y1, x1, y2, color);
        this.drawRect(context, paint, x1, y2 - 0.5f, x2, y2, color);
        this.drawRect(context, paint, x1, y1, x2, y1 + 0.5f, color);
        this.drawRect(context, paint, x2 - 0.5f, y1, x2, y2, color);
    }

    private void drawHealthBar(DrawContext context, Paint paint, LivingEntity entity, Vector4d v, Color color) {
        float healthFrac;
        ...
        this.drawRect(context, paint, barX - 0.6f, barY - 0.6f, barX + barW + 0.6f, barY + barH + 0.6f, Color.BLACK);
        this.drawRect(context, paint, barX, barY, barX + barW, barY + barH, color.darker().darker());
        Color healthColor = this.getHealthColor(healthFrac);
        if (position.equals("Left") || position.equals("Right")) {
            this.drawRect(context, paint, barX, barY + barH * (1.0f - healthFrac), barX + barW, barY + barH, healthColor);
        } else {
            this.drawRect(context, paint, barX, barY, barX + barW * healthFrac, barY + barH, healthColor);
        }
    }

    private void drawRect(DrawContext context, Paint paint, float left, float top, float right, float bottom, Color color) {
        paint.setColor(color.getRGB());
        context.drawRectXYWH(left, top, right - left, bottom - top, paint);
    }
