...
public class OpalNameTag extends NameTagStyle {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        for (ItemRenderData data : deferredItems) {
            Matrix3x2fStack stack = event.guiGraphics().pose();
            stack.pushMatrix();
            stack.translate(data.position.x, data.position.y);
    ...
    public void onPacket(PacketEvent packetEvent) {
        ...
        if (packet.owner().equals(mc.player.getGameProfile().getName())) return;
        scoreboardHealthMap.computeIfAbsent(packet.owner(), k -> new AtomicInteger()).set(packet.score());
    }
