...
public class Scaffold extends Module {
    ...
    public final BooleanSetting autoJump = new BooleanSetting("Auto Jump", false, () -> this.mode.is("Normal"));
...
//            mc.player.getInventory().selected = this.oldSlot;
    ...
    public void onTick(TickEvent event) {
        ...
        if (placeableSlot != -1 && mc.player.getInventory().getSelectedSlot() != placeableSlot) {
//            mc.player.getInventory().selected = placeableSlot;
            mc.player.getInventory().setSelectedSlot(placeableSlot);
        }
        ...
        if (this.mode.is("Normal")) {
            mc.options.keyJump.setDown(jumpHeld
                    || this.autoJump.getValue() && MovementUtil.isMoving());
        }
            ...
            ClientBase.delayPackets.add(() -> {
                ...
                if (previousTarget != null) {
                    if (previousTarget.getYaw() != rotationToBlock.getYaw()) {
                        PacketUtil.sendQueued(new ServerboundMovePlayerPacket.Rot(yaw, rotationToBlock.getPitch(), mc.player.onGround(), mc.player.horizontalCollision));
                    }
                } else {
                    PacketUtil.sendQueued(new ServerboundMovePlayerPacket.Rot(yaw, rotationToBlock.getPitch(), mc.player.onGround(), mc.player.horizontalCollision));
                }
    ...
    private boolean isPlacementReachable(PlacementTarget target) {
        ...
        Vec3 hitPoint = blockCenter.add(new Vec3(
                target.facing.getStepX() * 0.5,
                target.facing.getStepY() * 0.5,
                target.facing.getStepZ() * 0.5));
        ...
        return toBlock.lengthSqr() <= 20.25
                && toBlock.normalize().dot(target.facing.getUnitVec3().scale(-1.0)) >= 0.0;
    }
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (mc.level.getBlockState(offset).entityCanStandOnFace(mc.level, offset, mc.player, direction)) {
                ...
                if (delta.lengthSqr() <= 20.25
                        && delta.normalize().dot(direction.getUnitVec3()) >= 0.0) {
