...
public class NoSlow extends Module {
    ...
    public final ModeSetting mode             = new ModeSetting("Mode", "Grim V3", "NoSlow", "Vanilla").withDefault("Grim V3");
    ...
    public void onSlowdown(SlowdownEvent event) {
        ...
        if (anim == ItemUseAnimation.BOW && this.crossbowNoSlow.getValue()
                || anim == ItemUseAnimation.CROSSBOW && this.foodNoSlow.getValue()
                || this.isEatOrDrink(stack)
                || anim == ItemUseAnimation.BLOCK) {
            event.setSlowDown(false);
        }
        if (this.keepSprinting.getValue()) {
            mc.player.setSprinting(true);
    ...
    public void onPacket(PacketEvent event) {
        ...
        if (event.getPacket() instanceof ServerboundUseItemOnPacket useOnPacket
                ...
                && mc.player.getInventory().getSelectedSlot() == this.swapInitSlot) {
        ...
        if (event.getPacket() instanceof ServerboundUseItemPacket usePacket) {
            if (this.didSwapHand || this.releaseTicksRemaining > 0) {
                ...
                if (!this.timer.hasPassed(150.0f) && this.releaseTicksRemaining <= 0) {
                    ...
                    ItemUseAnimation anim = handStack.getUseAnimation();
                    if ((anim == ItemUseAnimation.BOW && this.crossbowNoSlow.getValue())
                            || (anim == ItemUseAnimation.CROSSBOW && !CrossbowItem.isCharged(handStack) && this.foodNoSlow.getValue())) {
                        this.shouldReleaseItem = false;
                        this.startBlink(1);
    ...
    private void startUseItem(InteractionHand hand, int count) {
        ...
        this.swapInitSlot = mc.player.getInventory().getSelectedSlot();
        this.useHand = hand == InteractionHand.MAIN_HAND ? InteractionHand.OFF_HAND : InteractionHand.MAIN_HAND;
        this.sendSwapOffhand();
        if (count > 0) {
            PacketUtil.sendQueued(new ServerboundUseItemPacket(this.useHand, count,mc.player.getXRot(),mc.player.getYRot()));
        } else {
            PacketUtil.sendPredictiveDirect(seq -> new ServerboundUseItemPacket(this.useHand, seq,mc.player.getXRot(),mc.player.getYRot()));
        }
        this.startBlink(2);
    }
    ...
    private boolean isFoodOrPotion(ItemStack stack) {
        ...
        ItemUseAnimation anim = stack.getUseAnimation();
        if (anim != ItemUseAnimation.EAT && anim != ItemUseAnimation.DRINK) return false;
        Item item = stack.getItem();
        return stack.has(DataComponents.FOOD) && this.potionNoSlow.getValue()
    ...
    private boolean isEatOrDrink(ItemStack stack) {
        ...
        ItemUseAnimation anim = stack.getUseAnimation();
        Item item = stack.getItem();
        return anim == ItemUseAnimation.EAT && this.potionNoSlow.getValue()
                || anim == ItemUseAnimation.DRINK && this.shieldNoSlow.getValue()
                || item instanceof PotionItem && this.shieldNoSlow.getValue();
    }

    private boolean isUseAnimation(ItemUseAnimation anim) {
        return anim == ItemUseAnimation.EAT || anim == ItemUseAnimation.DRINK || anim == ItemUseAnimation.BOW
                || anim == ItemUseAnimation.SPEAR || anim == ItemUseAnimation.CROSSBOW;
    }
    ...
    private Packet<?> createUseItemPacket(int sequence) {
        return new ServerboundUseItemPacket(this.useHand, sequence, mc.player.getXRot(),mc.player.getYRot());
    }
}
