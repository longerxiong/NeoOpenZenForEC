...
extends Module {
    ...
    public void onMotion(MotionEvent motionEvent) {
        ...
        if (motionEvent.isPost()) {
            if (mc.gameMode.isDestroying()) {
                int bestSlot;
                ItemStack heldStack;
                if (this.checkSword.getValue() && (heldStack = mc.player.getMainHandItem()).is(ItemTags.SWORDS)) {
                    return;
                }
                BlockHitResult blockHit;
                if (mc.hitResult != null && mc.hitResult.getType() == HitResult.Type.BLOCK && (bestSlot = this.getBestTool((blockHit = (BlockHitResult)mc.hitResult).getBlockPos())) != -1 && bestSlot != mc.player.getInventory().getSelectedSlot()) {
    ...
    private int getBestTool(BlockPos blockPos) {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (ItemUtil.isWeaponItem(itemStack) || itemStack.isEmpty() || blockState.isAir() || itemStack.is(ItemTags.SWORDS) && !(block instanceof WebBlock)) continue;
