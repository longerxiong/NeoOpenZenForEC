        ...
        if (rawPacket instanceof ClientboundAddEntityPacket addEntityPacket) {
            if (this.attackOnPacket.getValue() && addEntityPacket.getType() == EntityType.END_CRYSTAL) {
                ...
                if (mc.player.distanceTo(endCrystal) <= 4.0f) {
                    ...
                    mc.getConnection().send(new ServerboundMovePlayerPacket.PosRot(mc.player.getX(), mc.player.getY(), mc.player.getZ(), rotation.getYaw(), rotation.getPitch(), mc.player.onGround(), mc.player.horizontalCollision));
                    PacketUtil.sendPredictive(seq -> new ServerboundUseItemPacket(InteractionHand.MAIN_HAND, seq, mc.player.getYRot(), mc.player.getXRot()));
