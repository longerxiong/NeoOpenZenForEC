...
extends Module {
    ...
    public void onPacketBot(PacketEvent packetEvent) {
        ...
        if (packet instanceof ClientboundAddEntityPacket addPlayer && addPlayer.getType() == EntityType.PLAYER) {
            if (!suspectJoinTimes.containsKey(addPlayer.getId())) {
                return;
            }
            String botName = suspectNames.get(addPlayer.getId());
            if (this.debug.getValue()) {
                ChatUtil.print("Bot Detected! (" + botName + ")");
            }
            confirmedBotNames.put(addPlayer.getId(), botName);
            suspectJoinTimes.remove(addPlayer.getId());
            confirmedBotIds.add(addPlayer.getId());
            return;
        }
