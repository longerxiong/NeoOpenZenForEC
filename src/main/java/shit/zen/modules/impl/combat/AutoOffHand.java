...
public class AutoOffHand extends Module {
    ...
    private void equipOffhand(int slot) {
        ...
        final int savedSlot = mc.player.getInventory().getSelectedSlot();
        if (slot >= 9) {
            PacketUtil.sendQueued(new ServerboundSetCarriedItemPacket(slot));
            ...
            PacketUtil.sendQueued(new ServerboundSetCarriedItemPacket(mc.player.getInventory().getSelectedSlot()));
            PacketUtil.sendQueued(new ServerboundPlayerActionPacket(
                    ServerboundPlayerActionPacket.Action.SWAP_ITEM_WITH_OFFHAND, BlockPos.ZERO, Direction.DOWN));
