...
NeoOpenZen 作为 OpenZen 的移植版本，同样支持上述交付方式。

> **本项目不能作为 Forge mod 或 NeoForge mod 启动。** `mods/` 加载路径不被支持，不要把 jar 丢进 `.minecraft/mods/`。
...
> **本仓库暂未配置 Github Actions，有时间了整，请自行下载源码并配置编译环境，或下载 Release 的构建版本**
> - ⚠️ 目前 EaseCation 官方暂时没有安装像 Heypixel 的 Native 层反作弊和包名拉黑机制，若后续添加，则 Release 中的包名很可能被拉黑。
...
- **JDK 21**（推荐 Microsoft Build of OpenJDK / Temurin / Azul Zulu 任一）。
...
首次执行会从 NeoForgeMaven 下载 1.21.8 + NeoForge 21.8.52 的 mappings 和依赖，耗时几分钟到十几分钟，取决于网络。
...
产物：`build/libs/openzen-1.0.jar`。在 Forge 启动器的 JVM 启动参数里加上：

```
-javaagent:"完整\路径\到\openzen-1.0.jar" -Djdk.attach.allowAttachSelf=true
```
...
1. 用 HMCL / Forge 启动器正常启动 Minecraft 1.21.8 NeoForge（**不需要**任何特殊 JVM 参数）。
...
### EaseCation 反作弊绕过
截至到目前(2026/07/19)，EaseCation Java 仅上线 Staff AC ，并不检测注入，可以放心游玩。  
